...
(function ($) {
		...
		var observer = new MutationObserver(function (mutations) {
			mutations.forEach(function (mutation) {
				mutation.addedNodes.forEach(function (node) {
					...
					// WordPress appends a <tr> or a wrapping element; the password
					// display lives inside .new-application-password.
					var newPassEl = node.classList && node.classList.contains('new-application-password')
						? node
						: node.querySelector && node.querySelector('.new-application-password');

					if (newPassEl) {
						injectConnectButton(newPassEl);
					}
				});
			});
		});
	...
	/**
	 * Inject the "Connect to AI" button inside a .new-application-password element.
	 *
	 * @param {Element} newPassEl
	 */
	function injectConnectButton(newPassEl) {
		if (newPassEl.querySelector('.mcp-helper-connect-btn')) {
		...
		btn.addEventListener('click', function () {
			var codeEl = newPassEl.querySelector('.code');
			currentPassword = codeEl ? codeEl.textContent.trim() : '';
			openModal();
		});

		// Append after the code display (which holds the password).
		var codeEl = newPassEl.querySelector('.code');
		if (codeEl && codeEl.parentNode) {
			codeEl.parentNode.insertBefore(btn, codeEl.nextSibling);
		} else {
			newPassEl.appendChild(btn);
		}
	}
