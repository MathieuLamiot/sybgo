 ...
 *
 * Usage:
 *   Mcp_Logger::log( 'TOKEN', 'grant received', [ 'grant_type' => $gt ] );
 *
 * Grep all MCP logs:
...
class Mcp_Logger {
	...
	/**
	 * Maximum length of any single header value written to logs.
	 *
	 * Authorization header values are truncated to this length to prevent
	 * accidental JWT exposure.
	 */
	const HEADER_TRUNCATE = 40; // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedConstantFound
	...
	/**
	 * Capture and sanitise all HTTP request headers for logging.
	 *
	 * - Authorization values are truncated: first 20 + last 8 chars with '...' in the middle.
	 * - Cookie header is replaced with '<redacted>'.
	 * - All other values are preserved as-is.
	 *
	 * @return array<string, string> Sanitised header map (lowercase names).
	 */
		...
		if ( function_exists( 'getallheaders' ) ) {
			...
			if ( is_array( $raw ) ) {
				foreach ( $raw as $name => $value ) {
					...
					$headers[ $key ] = self::sanitise_header( $key, (string) $value );
				}
			}
		}
	...
	/**
	 * Sanitise a single header value.
	 *
	 * @param string $name  Lowercase header name.
	 * @param string $value Header value.
	 * @return string Safe value for log output.
	 */
	private static function sanitise_header( string $name, string $value ): string {
		if ( 'cookie' === $name ) {
			return '<redacted>';
		}
	...
	private static function redact_sensitive_fields( array $data ): array {
		$sensitive = array( 'code_verifier', 'client_secret', 'access_token', 'refresh_token', 'password' );
